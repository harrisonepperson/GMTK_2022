...
public class Player : RigidBody
{
	...
	private RayCast eyes;

	public override void _Ready()
	{
		eyes = GetNode<RayCast>("Eyes");

		GD.Load<PackedScene>(deathScreen);
		actionTimer = GetNode<Timer>("ActionLock");

		movementGrid = GetNode<GridContainer>("CanvasLayer/FullScreen/Bottom UI/MovementGrid");
	...
	public void _on_action_light_attack_pressed()
	{
		if (autoload.isPlayerTurn && !moveLock && !actionLock && remainingActions > 0)
		{
			...
			if (enemies.Count > 0)
			{
				...
				float closestDist = GlobalTransform.origin.DistanceSquaredTo(closestEnemy.GlobalTransform.origin);

				foreach (Enemy e in enemies)
				{
					if (!e.isDead)
					{
						float enemDist = GlobalTransform.origin.DistanceSquaredTo(e.GlobalTransform.origin);
				...
				if (closestDist <= 8)
				{
					ball.Translation = new Vector3(closestEnemy.GlobalTransform.origin.x - GlobalTransform.origin.x, 2, closestEnemy.GlobalTransform.origin.z - GlobalTransform.origin.z);
					ball.Emitting = true;
		...
		if (autoload.isPlayerTurn && !moveLock && !actionLock && remainingActions > 0)
		{
			...
			if (enemies.Count > 0)
			{
				...
				foreach (Enemy e in enemies)
				{
					if (!e.isDead)
					{
						float dist = GlobalTransform.origin.DistanceSquaredTo(e.GlobalTransform.origin);
	...
	public void _on_action_range_attack_pressed()
	{
		if (autoload.isPlayerTurn && !moveLock && !actionLock && remainingActions > 0)
		{
			...
			if (enemies.Count > 0)
			{
				...
				if (closestDist <= 36)
				{
					fireBall.castTo(new Vector3(closestEnemy.GlobalTransform.origin.x - GlobalTransform.origin.x, 2, closestEnemy.GlobalTransform.origin.z - GlobalTransform.origin.z));
